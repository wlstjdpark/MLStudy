...
> [Machine Learning is the] field of study that gives computers the ability to learn without being explicitly programmed. Arthur Samuel, 1959</cite>

> A computer program is said to learn from experience E with respect to some task T and some performance measure P, if its performance on T, as measured by P, improves with experience E. Tom Mitchell, 1997</cite>
...
- 머신러닝 시스템을 분류하는 기준
    ...
    - 학습 데이터의 패턴을 감지하고 예측 모델을 작성해 작업하거나 (모델 기반 학습)
...
- 학습을 위해 알고리즘에 제공하는 학습 데이터에 원하는 결과를 "레이블(Label)"로 달아둔다.
...
- 학습을 위해 알고리즘에 제공되는 학습 데이터에 레이블을 달아두지 않는다.
...
- 일반적으로 반지도 학습을 많이 사용하는 작업
    - 심층 신뢰 신경망 (Deep Belief Network; DBN)
        ...
        - RBM은 비지도 학습 방식을 통해 순차적으로 학습되고, 시스템 전체는 지도 학습 방식을 사용해 미세하게 조정한다.
...
- 아기에게 사과가 무엇인지 가르치려면, 사과를 가리켜 "사과"라고 말하면 된다 (몇 번 반복하면 아마도 배울 것이다).
...
- 학습 데이터가 일반화하려는 새로운 사례를 대표하는게 중요하다 (인스턴스 기반 학습, 모델 기반 학습 모두).
    - 예를 들어, 선형 모델을 학습하려고 이전에 사용했던 국가의 집합은 대표성을 완벽하게 띠지 않는다. 몇 개의 국가가 빠져 있다.
...
- 그래프 분석
    ...
    - 대표성을 띠지 않는 학습 집합으로 인해, 매우 빈곤하거나 매우 부유한 국가의 경우 정확한 예측을 내릴 수 없는 모델을 학습했다.

- 일반화하려는 사례에 대해 대표성을 띠는 학습 집합을 사용하는 것은 매우 중요하다.
...
- 학습 데이터에 오류, 특이점 및 노이즈가 가득차 있다면 시스템이 기본 패턴을 감지하기 더 어려워지므로 시스템 성능이 떨어지게 된다.
    - 따라서 학습 데이터를 정리하는데 많은 시간을 할애해야 한다.
    - 사실, 대부분의 데이터 과학자들은 학습 데이터를 정리하는데 많은 소비한다. 예를 들어,
...
- "콩 심은 데 콩 나고, 팥 심은 데 팥 난다"라는 속담이 있다.
    - 시스템은 학습 데이터에 관련성이 충분하고, 무관한 데이터가 너무 많지 않은 경우에만 학습할 수 있다.
    - 머신러닝 프로젝트의 성공에 있어서 결정적으로 중요한 부분은 학습할 수 있는 좋은 특징 집합을 갖는 것이다.

- 특징 공학(Feature Engineering)은 다음을 포함한다.
...
- 외국을 방문했는데 택시 운전사가 여러분에게 바가지를 씌웠다고 가정하자.
    ...
    - 머신러닝에서는 이를 과적합이라고 한다. 즉, 모델이 학습 데이터는 잘 학습하지만 일반화는 잘 되지 않는 경우를 말한다.

- 아래 그림은 학습 데이터에 강하게 부합하는 고차 다항식으로 된 삶의 만족도 모델의 예를 보여준다.

![](https://github.com/utilForever/MLStudy/blob/master/Resources/mlst_0122.png)

- 단순한 선형 모델보다 학습 데이터에서 훨씬 나은 성능을 보여준다. 하지만 예측을 정말로 신뢰할 수 있을까?
    - 심층 신경망과 같은 복합 모델은 데이터의 미묘한 패턴을 감지할 수 있다.
    - 하지만 학습 집합에 노이즈가 있거나 데이터가 너무 적으면(샘플링 노이즈를 발생시킴) 노이즈 자체의 패턴을 감지할 수도 있다.
        - 이러한 패턴은 새로운 인스턴스로 일반화되지 않는다.

- 예를 들어, 국가의 이름과 같은 정보가 없는 속성을 포함해 더 많은 속성을 삶의 만족도 모델에 제공한다고 가정해 보자.
    - 복합 모델은 이름에 "w"가 있는 학습 데이터의 모든 국가에서 삶의 만족도가 7보다 크다는 패턴을 감지할 수 있다.
        - 뉴질랜드(7.3), 노르웨이(7.4), 스웨덴(7.2), 스위스(7.5)
    - 하지만 W-만족 규칙이 르완다나 짐바브웨에 일반화되어 있다고 확신하는가?
        - 이 패턴은 학습 데이터에서 순수한 기회를 통해 발생했다.
        - 그러나 모델은 패턴이 실제인지, 또는 단순히 데이터의 노이즈 결과인지의 여부를 알 수 있는 방법이 없다.

- 모형을 단순하게 만들고 과적합의 위험을 줄이는 방법이 있는데, 이를 정규화(Regularization)라고 한다.
    - 예를 들어, 앞에서 정의한 선형 모델에는 θ0와 θ1이라는 두 개의 매개 변수가 있다.
        - 이를 통해 모델을 학습 데이터에 적용할 수 있도록 학습 알고리즘에 자유도(Degree of Freedom)가 2인 모델을 제공한다.
            - 선의 높이(θ0)와 기울기(θ1)를 조정할 수 있다.
        - θ1 = 0을 강요하면, 알고리즘의 자유도는 1이 되며 데이터를 적절하게 맞추기 더 힘들어질 것이다.
        - 학습 인스턴스에 최대한 근접하도록 라인을 위나 아래로 이동하는 작업을 해야 한다. 정말로 단순한 모델이다!
        - 알고리즘에서 θ1을 수정할 수 있게 해주지만, 작게 유지해야 한다면 실제로 학습 알고리즘의 자유도는 1에서 2 사이를 갖는다.
            - 이를 통해 자유도가 2인 모델보다 더 간단한 모델을 생성하겠지만, 자유도가 1인 모델보다는 복잡하다.
        - 데이터를 완벽하게 맞추는 것과 모델이 잘 일반화 될 수 있도록 모델을 단순하게 유지하는 것 사이에서 올바른 균형을 맞춰야 한다.

![](https://github.com/utilForever/MLStudy/blob/master/Resources/mlst_0123.png)        

- 위 그림은 세 가지 모델을 보여준다.
    - 점선은 몇 개 국가를 뺀 상태에서 학습한 기존 모델을 나타낸다.
    - 파선은 모든 국가를 학습한 모델을 나타낸다.
    - 실선은 첫번째 모델과 동일한 데이터로 학습하되, 정규화 제약 조건을 적용한 모델을 나타낸다.
- 정규화로 인해 모델이 더 작은 기울기를 갖게 되어 모델에서 학습한 데이터에는 덜 적합하지만, 실제로운 새로운 예제에 더 잘 일반화된다.

- 학습 중에 적용할 정규화의 양은 초모수(Hyperparameter)로 제어할 수 있다.
    - 초모수는 모델이 아닌 학습 알고리즘의 매개 변수다.
    - 따라서 학습 알고리즘 자체의 영향을 받지 않는다.
    - 학습 전에 설정해야 하며 학습 중에도 일정해야 한다.
    - 정규화 초모수를 매우 큰 값으로 설정하면, 거의 평평한(기울이가 0에 가까운) 모델이 나온다.
    - 초모수를 사용하면 학습 알고리즘은 대부분의 경우 과적합하지 않게 되지만, 좋은 해결책을 찾기도 어려워진다.
    - 초모수를 조정하는 작업은 머신러닝 시스템을 구축하는데 매우 중요한 부분을 차지한다.

##### 학습 데이터의 과소적합 (Underfitting)

- 과적합의 반댓말이다. 모델이 너무 단순해 데이터의 기본 구조를 학습할 수 없는 경우에 발생한다.
    - 예를 들어, 삶의 만족도에 대한 선형 모델은 과소적합한 경향이 있다.
    - 현실은 모델보다 훨씬 복잡하므로, 학습 예제에서도 예측이 부정확할 수 있다.

- 과소적합 문제를 해결하기 위한 주요 선택 사항은 다음과 같다.
    - 더 많은 매개 변수로 보다 강력한 모델 선택하기
    - 학습 알고리즘에 더 나은 특징을 제공하기 (특징 공학)
    - 모델 상의 제약을 감소시키기 (예를 들어, 정규화 초모수 감소시키기)

### 테스트와 검증

- 모델이 새로운 사례에 대해 잘 일반화되는지 확인할 수 있는 방법은 실제로 시도해보는 방법 뿐이다.
    - 이를 수행하는 한 가지 방법으로 모델을 실제로 투입하고 성능을 모니터링하는 방법이 있다.
        - 이 방법은 잘 동작하지만, 모델이 심각하게 좋지 않을 경우 사용자가 불만을 토로하게 된다.
        - 따라서 가장 좋은 방법은 아니다.
    - 더 나은 방법으로 데이터를 학습 집합과 테스트 집합, 두 집합으로 분리하는 방법이 있다.
        - 학습 집합을 사용해 모델을 학습시키고, 테스트 집합을 사용해 모델을 테스트한다.
        - 새 사례에서의 오류율을 일반화 오류(Generalization Error)라고 한다.
            - 표본 외 오류(Out-of-Sample Error)라고도 한다.
            - 테스트 집합에서 모델을 테스트해 이 오류를 추정할 수 있다.
            - 이 값은 전에 보지 못한 인스턴스에서 모델이 얼마나 잘 수행되는지 알려준다.
    - 학습 오류가 낮지만(예를 들어, 모델에서 학습 집합에 대해 실수를 거의 하지 않지만) 일반화 오류가 높다면?
        - 모델이 학습 데이터에 적합하지 않다는 것을 의미한다.
- 따라서 모델을 계산하는 것만으로도 충분하다. 테스트 집합을 사용하길 바란다.

- 이제 두 모델(선형 모델과 다항 모델) 사이에서 무엇을 사용할 것인지 주저하고 있다고 가정하자. 어떻게 결정할 수 있을까?
    - 한 가지 선택 사항으로 두 모델을 학습시키고 테스트 집합을 사용해 얼마나 잘 일반화하는지 비교해 보는 방법이 있다.
    - 선형 모델이 더 잘 일반화한다고 가정하자. 과적합하지 않도록 정규화를 일부 적용하려고 한다.
    - 정규화 초모수의 값은 어떻게 선택할 것인가?
        - 한 가지 선택 사항으로 초모수에 100가지 값을 사용해 100가지의 모델을 학습시키는 방법이 있다.
        - 가장 낮은 일반화 오류(예를 들어, 5% 오차)를 갖는 모델을 생성하는 최상의 초모수 값을 찾았다고 가정하자.
        - 이 값을 선택한 모델을 실제로 투입했는데, 불행하게도 예상대로 성능이 떨어지고 15%의 오차가 발생했다.
        - 원인이 무엇일까?
            - 테스트 집합에서 일반화 오류를 여러 번 측정하고 모델과 초모수를 적용해 해당 집합에 대한 최상의 모델을 생성했기 때문이다.
            - 이 말은 모델이 새로운 데이터에서 잘 수행되지 않음을 의미한다.
        - 해결책은 검증 집합(Validation Set)이라고 하는 두 번째 검정 표본을 만드는 것이다.
            - 학습 집합을 사용해 다양한 초모수로 여러 모델을 학습하고, 검증 집합에서 가장 잘 수행되는 모델과 초모수를 선택한다.
            - 모델이 만족스럽다면, 테스트 집합에서 최종 테스트를 수행해 일반화 오류를 측정한다.

- 검증 집합에서 너무 많은 학습 데이터를 "낭비"하지 않으려면, 일반적으로 교차 검증(Cross-Validation)이라는 기법을 사용한다.
    - 학습 집합을 여집합 관계를 갖는 하위 집합으로 분할한다.
    - 각 모델은 부분 집합에서 몇 개의 부분 집합을 선택해 학습한 뒤, 나머지 부분 집합에 대해 검증을 한다.
    - 모델 유형과 초모수를 선택하고 나면, 전체 학습 집합에서 초모수를 사용해 최종 모델을 학습하고 테스트 집합에서 일반화 오류를 측정한다.
